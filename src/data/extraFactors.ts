...
// Generate the same factors as the main factors page
const generateExtraFactors = (): Factor[] => {
  ...
  factorTypes.forEach(({ name, trait, bonusValues }) => {
    for (let level = 1; level <= 9; level++) {
      factors.push({
        id: `extra_factor_${name.toLowerCase()}_${level}`,
        name,
        trait,
        level,
        maxLevel: 9,
...
export const extraFactors: Factor[] = generateExtraFactors();

// Debug log to check what's generated
console.log('Extra Factors generated:', extraFactors.length, 'factors');
console.log('Factor types:', [...new Set(extraFactors.map(f => f.name))]);
console.log('All factor names:', extraFactors.map(f => f.name));
console.log('All factor traits:', extraFactors.map(f => f.trait)); 